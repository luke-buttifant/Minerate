...
window.onload = function() {
    ...
    chrome.storage.sync.get([
        ...
        if (!chrome.runtime.error && items.storedID != undefined) {
            ...
            var r_24Reward = (data['24hreward'] / 100000000).toFixed(6);
            var r_balance = (data['stats']['balance'] / 100000000).toFixed(6);
            var r_hashrate = (data['currentHashrate'] / 1000000).toFixed(2);
            if (r_hashrate > 1000000) {
                r_hashrate = (r_hashrate / 1000000).toFixed(2);
                frequency = 'TH/s';
            } else if (r_hashrate > 1000 && r_hashrate < 100000) {
    ...
    requestBtn.onclick = async ()=>{
        ...
        try {
            ...
            const response = await fetch(request);
            const data = await response.json();
            var r_24Reward = (data['24hreward'] / 100000000000).toFixed(6);
...
//# sourceMappingURL=popup.6f8cee5f.js.map
