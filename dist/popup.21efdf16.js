...
window.onload = function() {
    ...
    requestBtn.onclick = async ()=>{
        ...
        for(var i = 0; i < serversLength; i++)var server = fetch(server + baseUrl + id).then((response)=>{
            if (response.ok) {
            ...
            } else if (response.status === 404 && serversLength == 0) {
                alert.classList.remove("hidden");
                select.classList.remove("hidden");
                optionsLbl.classList.remove("hidden");
                return Promise.reject('No Server Match');
...
//# sourceMappingURL=popup.21efdf16.js.map
