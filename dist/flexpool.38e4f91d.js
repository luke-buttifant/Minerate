window.onload = function() {
    ...
    chrome.storage.sync.get('flexStoredID', async function(result) {
        if (result.flexStoredID != undefined) {
            ...
            response = await fetch(request);
            data = await response.json();
            var r_hashrate = data['result']['currentEffectiveHashrate'] / 1000000;
            ...
            console.log(r_hashrate);
            if (r_hashrate > 1000000000000) {
                r_hashrate = (r_hashrate / 1000000000).toFixed(2);
                frequency = 'TH/s';
            } else if (r_hashrate > 1000000 && r_hashrate < 1000000000000) {
                r_hashrate = (r_hashrate / 1000000000).toFixed(2);
                frequency = 'GH/s';
            } else {
...
//# sourceMappingURL=flexpool.38e4f91d.js.map
